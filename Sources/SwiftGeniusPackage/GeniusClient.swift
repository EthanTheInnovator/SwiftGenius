...
open class GeniusClient: AuthorizedJSONClient, Genius {
    ...
    public init(consumerKey: String,
        ...
        oAuth = OAuth2Swift(consumerKey: consumerKey,
                            ...
                            responseType: "code")
        super.init(oAuth: oAuth, authorizeUrl: authUrl.absoluteString, baseUrl: geniusBaseUrl, jsonDecoder: JSONDecoder())
    }
    ...
    open func authorize() -> Promise<String> {
        return Promise<String> { (seal) in
            ...
            _ = oAuth.authorize(withCallbackURL: URL(string: "immediate://oauth-callback/genius")!, scope: scopeString, state: "code", completionHandler: { (result) in
                do {
                    let tokenSuccess = try result.get()
                    self.oAuthToken = tokenSuccess.credential.oauthToken
                    print(tokenSuccess.credential.oauthToken)
                    seal.fulfill(tokenSuccess.credential.oauthToken)
                }
                catch {
                    seal.reject(error)
                }
